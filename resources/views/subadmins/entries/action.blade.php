<span style="overflow: visible; position: relative; width: 110px;">
	...
	@if($tracking->status != 'deported')
		@if($table_type == '0')
		<a href="{{ route('admin-trackings.status-ported', $tracking->id)}}" title="Mark as Ported" class="btn btn-sm btn-success btn-icon btn-icon-md">
			<i class="fas fa-check-square"></i>&nbsp;&nbsp;Ported
		</a>
		@else
		<a href="{{ route('admin-trackings.status-deported', $tracking->id)}}" title="Mark as Deported" class="btn btn-sm btn-danger btn-icon btn-icon-md">
